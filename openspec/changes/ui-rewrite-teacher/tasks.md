...
- [ ] 1.1 Create `src/components/teacher/shared/` directory structure
- [ ] 1.2 Create `src/components/teacher/shared/types.ts` with shared TypeScript types
- [ ] 1.3 Create `src/components/teacher/shared/PageHeader.tsx` component for consistent page headers
- [ ] 1.4 Create `src/components/teacher/shared/TeacherFilterBar.tsx` compound component with ClassSelect, SubjectSelect, and DatePicker sub-components

## 2. Grade Picker Component

- [ ] 2.1 Create `src/components/teacher/grades/GradePicker.tsx` as a controlled component with value, onChange, disabled, and config props
- [ ] 2.2 Implement the 1-6 grade grid with color-coded cells matching current grade colors
- [ ] 2.3 Implement modifier buttons (+, -, 0) with grade value computation logic
...
- [ ] 3.2 Create `src/components/teacher/grades/AddPeriodGradeModal.tsx` reusing GradePicker with semester selector
- [ ] 3.3 Migrate all validation rules from existing modals to new implementations
- [ ] 3.4 Verify visual parity with existing modals (same layout, styling, animations)
- [ ] 3.5 Test grade submission flow for both modals against the API

## 4. Student Grade Table

- [ ] 4.1 Create `src/components/teacher/grades/StudentGradeTable.tsx` using the shared Table primitive
- [ ] 4.2 Implement student row with name, recent grade badges, and action buttons
- [ ] 4.3 Implement grade badge color coding using existing `getGradeColor` utility
- [ ] 4.4 Add loading, error, and empty state handling
...
- [ ] 6.1 Create new `src/components/teacher/grades/TeacherGradesPage.tsx` composing PageHeader, TeacherFilterBar, StudentGradeTable, and modals
- [ ] 6.2 Migrate filter state management from monolithic component to new structure
- [ ] 6.3 Migrate data fetching hooks (useQuery for classes, subjects, students, grades)
- [ ] 6.4 Replace old TeacherGradesPage in App.tsx routes
