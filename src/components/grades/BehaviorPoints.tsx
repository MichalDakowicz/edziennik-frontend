...
export default function BehaviorPoints({ behavior }: { behavior: BehaviorPoint[] }) {
  ...
  return (
    <div className="space-y-4">
      <div className={`rounded-[var(--radius)] border border-border p-4 ${total >= 0 ? "bg-emerald-100 dark:bg-emerald-900/20 border-emerald-200 dark:border-emerald-800/50" : "bg-red-100 dark:bg-red-900/20 border-red-200 dark:border-red-800/50"}`}>
      ...
      <div className="space-y-2">
        {sorted.map((item) => (
          <div key={item.id} className="bg-card border border-border rounded-[var(--radius)] p-3 flex justify-between gap-3">
            <div>
              <p className="text-foreground">{item.opis ?? "Brak opisu"}</p>
              <p className="text-xs text-muted-foreground tabular-nums">{formatDateTime(item.data_wpisu)}</p>
            </div>
