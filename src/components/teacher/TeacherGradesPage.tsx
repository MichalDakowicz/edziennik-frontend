...
function RecentGradesCell({
    ...
    const { data: grades, isLoading } = useQuery({
        queryKey: selectedSubjectId
            ? keys.grades(studentId)
            : ["grades-disabled", studentId],
    ...
    const filtered = grades
        ...
        .sort(
            (a, b) =>
                new Date(b.data_wystawienia).getTime() -
                new Date(a.data_wystawienia).getTime(),
        );
                ...
                <div key={g.id} className="relative group">
                    ...
                    <div className="pointer-events-none absolute bottom-full left-1/2 z-50 mb-1 w-max -translate-x-1/2 rounded-md  bg-popover px-2 py-1 text-[10px] leading-tight text-popover-foreground opacity-0 shadow-sm transition-opacity duration-150 group-hover:opacity-100">
                        ...
                        <div>
                            ...
                            {formatDateTime(g.data_wystawienia)}
                        </div>
                    </div>
                </div>
            ))}
...
export const getBehaviorGradeColor = (gradeStr: string) => {
    if (!gradeStr)
        return "bg-surface-container text-on-surface border border-transparent";
    const lower = gradeStr.toLowerCase();
    if (lower.includes("6") || lower.includes("wzorowe"))
        return "bg-emerald-100 text-emerald-800 border border-emerald-200 dark:bg-emerald-900/20 dark:text-emerald-400 dark:border-emerald-800/50";
    if (lower.includes("5") || lower.includes("bardzo dobre"))
        return "bg-green-100 text-green-800 border border-green-200 dark:bg-green-900/20 dark:text-green-400 dark:border-green-800/50";
    if (
        lower.includes("4") ||
        (lower.includes("dobre") && !lower.includes("bardzo"))
    )
        return "bg-blue-100 text-blue-800 border border-blue-200 dark:bg-blue-900/20 dark:text-blue-400 dark:border-blue-800/50";
    if (lower.includes("3") || lower.includes("poprawne"))
        return "bg-yellow-100 text-yellow-800 border border-yellow-200 dark:bg-yellow-900/20 dark:text-yellow-400 dark:border-yellow-800/50";
    if (lower.includes("2") || lower.includes("nieodpowiednie"))
        return "bg-orange-100 text-orange-800 border border-orange-200 dark:bg-orange-900/20 dark:text-orange-400 dark:border-orange-800/50";
    if (lower.includes("1") || lower.includes("naganne"))
        return "bg-red-100 text-red-800 border border-red-200 dark:bg-red-900/20 dark:text-red-400 dark:border-red-800/50";
    return "bg-surface-container text-on-surface border border-transparent";
};

export default function TeacherGradesPage() {
    const { selectedClassId: hookClassId, setSelectedClassId: setHookClassId } =
        useTeacherClassSelector();
    const [activeGradeStudentId, setActiveGradeStudentId] = useState<
        number | null
    >(null);
    const [isAddPeriodGradeModalOpen, setIsAddPeriodGradeModalOpen] =
        useState(false);
    ...
    const [pendingGrades, setPendingGrades] = useState<
        Record<
            number,
            { value: string; base: number; modifier: "+" | "-" | "" }
        >
    >({});
    const [pendingBehaviorPoints, setPendingBehaviorPoints] = useState<
        Record<number, number>
    >({});
    const [gradeMenuAnchor, setGradeMenuAnchor] = useState<{
        top: number;
        left: number;
    } | null>(null);
    const [gradeMenuPosition, setGradeMenuPosition] = useState<{
        top: number;
        left: number;
    } | null>(null);
    const [proposedGrades, setProposedGrades] = useState<
        Record<number, string>
    >({});
    const [finalGrades, setFinalGrades] = useState<Record<number, string>>({});
    const [periodGradesMap, setPeriodGradesMap] = useState<
        Record<number, PeriodGrade[]>
    >({});
    ...
    const createPeriodGradeBatchMutation = useMutation({
        mutationFn: async (
            payload: {
                studentId: number;
                value: string;
        ...
        onSuccess: (_, variables) => {
            ...
            variables.forEach((v) => {
                queryClient.invalidateQueries({
                    queryKey: ["period-grades", v.studentId],
                });
            });
    ...
    const setPendingGrade = (
        studentId: number,
        grade: { value: string; base: number; modifier: "+" | "-" | "" },
    ) => {
        setPendingGrades((prev) => ({ ...prev, [studentId]: grade }));
    };

    const setPendingBehaviorPoint = (
        studentId: number,
        points: number | null,
    ) => {
    ...
    // Load period grades and regular/behavior grades
    useEffect(() => {
        if (!selectedClassId) return;
        if (gradeMode === "period" && !selectedSubjectId) return;
        if (gradeMode !== "period" && gradeMode !== "behavior") return;
        ...
        Promise.all([
            Promise.all(
                studentIds.map(async (id: number) => {
                    ...
                    return {
                        studentId: id,
                        grades: grades.filter((g: PeriodGrade) =>
                            gradeMode === "behavior"
                                ? g.przedmiot === null
                                : g.przedmiot === selectedSubjectId,
                        ),
                    };
                }),
            ),
            ...
            gradeMode === "behavior"
                ? Promise.all(
                      studentIds.map(async (id: number) => {
                          const behavior = await getBehaviorPoints(id);
                          return { studentId: id, behavior };
                      }),
                  )
                : Promise.resolve([]),
        ]).then(([periodResults, gradeResults, behaviorResults]) => {
            const periodMap: Record<number, PeriodGrade[]> = {};
            periodResults.forEach((r) => {
                periodMap[r.studentId] = r.grades;
            });
            ...
            if (gradeMode === "period") {
                ...
                setBehaviorByStudent(behaviorMap);
            }
    ...
    const canShowStudents = Boolean(
        selectedClassId && (gradeMode === "behavior" || selectedSubjectId),
    );
    ...
    useEffect(() => {
        ...
        function handleClickOutside(event: MouseEvent) {
            ...
            const clickedMenu =
                !!gradeMenuRef.current && gradeMenuRef.current.contains(target);
            const clickedButton =
    ...
    const getGradeStyles = (num: number | null, mod: "+" | "-" | "") => {
        if (num === null)
            return "bg-surface-container-highest text-on-surface-variant font-body border-border";

        const value = getGradeValue(num, mod);

        if (value >= 5.0)
            return "bg-emerald-100 text-emerald-800 border-emerald-200 dark:bg-emerald-900/20 dark:text-emerald-400 dark:border-emerald-800/50";
        if (value >= 4.0)
    ...
    if (studentsLoading || subjectsLoading || classesLoading)
        return <Spinner label="Ładowanie danych nauczyciela..." />;
    if (studentsError)
        return (
            <ErrorState message={`Błąd: ${(studentsError as Error).message}`} />
        );
    if (subjectsError)
        return (
            <ErrorState message={`Błąd: ${(subjectsError as Error).message}`} />
        );
    if (classesError)
        return (
            <ErrorState message={`Błąd: ${(classesError as Error).message}`} />
        );
    ...
    return (
        <div className="space-y-6">
                    ...
                    <div className="grid gap-4 md:grid-cols-[minmax(0,1.5fr)_minmax(0,1.5fr)] lg:grid-cols-[minmax(0,1.3fr)_minmax(0,1.3fr)_minmax(0,1.2fr)]">
                        ...
                        <div className="space-y-3">
                            <div>
                                ...
                                <div className="inline-flex rounded-lg p-1 border bg-zinc-900/40 border-zinc-800">
                                    {[1, 2, 3, 4, 5].map((w) => (
                                        <button
                                            key={w}
                                            type="button"
                            ...
                            <div className="space-y-1.5 rounded-lg border border-zinc-800 bg-zinc-950/40 px-3 py-2">
                                <label className="flex items-center justify-between gap-2 text-xs">
                                    ...
                                    <input
                                        ...
                                        onChange={(e) =>
                                            setGradeDoSredniej(e.target.checked)
                                        }
                                ...
                                <label className="flex items-center justify-between gap-2 text-xs">
                                    ...
                                    <input
                                        ...
                                        checked={gradePunkty}
                                        onChange={(e) =>
                                            setGradePunkty(e.target.checked)
                                        }
                                        className="w-4 h-4 cursor-pointer"
                                    />
                                </label>
                                <label className="flex items-center justify-between gap-2 text-xs">
                                    ...
                                    <input
                                        ...
                                        onChange={(e) =>
                                            setGradeOpisowa(e.target.checked)
                                        }
                                        className="w-4 h-4 cursor-pointer"
                                    />
                                </label>
            ...
            {(gradeMode === "period" || gradeMode === "behavior") && (
                    ...
                    {gradeMode === "behavior" && (
                        <div>
                            ...
                            <input
                                ...
                                placeholder="np. Aktywność na zajęciach..."
                                className="w-full bg-surface-container-lowest border border-outline-variant/20 rounded-xl py-2.5 px-4 text-sm focus:ring-2 focus:ring-primary/20 transition-all outline-none text-on-surface"
                            />
                        </div>
                    )}
                                ...
                                <table className="w-full text-left border-collapse">
                                    <thead>
                                        <tr className="bg-surface-container-low/50">
                                            ...
                                            <th className="px-6 py-5 text-xs font-bold text-on-surface-variant uppercase tracking-wider">
                                                {gradeMode === "behavior"
                                                    ? "Akcje / Punkty b."
                                                    : "Oceny bieżące"}
                                            </th>
                                            <th className="px-6 py-5 text-xs font-bold text-on-surface-variant uppercase tracking-wider text-center">
                                                {gradeMode === "behavior"
                                                    ? "Suma pkt"
                                                    : "Średnia"}
                                            </th>
                                    ...
                                    <tbody className="divide-y-0">
                                        {filteredStudents.map(
                                            (student: any, index: number) => {
                                                ...
                                                const getBehaviorSuggested = (
                                                    pts: number,
                                                ) => {
                                                    if (pts >= 200)
                                                        return "6 - Wzorowe";
                                                    if (pts >= 150)
                                                        return "5 - Bardzo dobre";
                                                    if (pts >= 100)
                                                        return "4 - Dobre";
                                                    if (pts >= 50)
                                                        return "3 - Poprawne";
                                                    if (pts >= 0)
                                                        return "2 - Nieodpowiednie";
                                                    return "1 - Naganne";
                                                };
                                                ...
                                                const suggested =
                                                    gradeMode === "period"
                                                        ? average > 0
                                                            ...
                                                            : null
                                                        : _behavior.length > 0
                                                          ? getBehaviorSuggested(
                                                                totalBehaviorPoints,
                                                            )
                                                          : null;

                                                const period1 = periodGradesMap[
                                                    student.id
                                                ]?.find((g) => g.okres === 1);
                                                return (
                                                    <tr
                                                    ...
                                                    >
                                                            ...
                                                            "behavior" ? (
                                                                <div className="inline-flex items-center justify-end gap-2">
                                                                    <button
                                                                        type="button"
                                                                        onClick={() =>
                                                                            setPendingBehaviorPoint(
                                                                                student.id,
                                                                                (pendingBehaviorPoints[
                                                                                    student
                                                                                        .id
                                                                                ] ??
                                                                                    0) -
                                                                    ...
                                                                    <input
                                                                        ...
                                                                        className="input-base h-9 w-16 text-center"
                                                                        placeholder="np. 5"
                                                                    />
                                                                    <button
                                                                        type="button"
                                                                            ...
                                                                            .map(
                                                                                (
                                                                                    ...
                                                                                    <span
                                                                                        ...
                                                                                        className={`w-7 h-7 flex items-center justify-center rounded-lg text-xs font-bold ${getGradeColor(g.wartosc)}`}
                                                                                    >
                                                                                        {formatGradeValue(
                                                                                            g.wartosc,
                                                                                        )}
                                                        ...
                                                        <td className="px-6 py-6 text-center">
                                                            ...
                                                            null ? (
                                                                ...
                                                                "period" ? (
                                                                    <span
                                                                        className={`inline-flex items-center justify-center w-10 h-10 rounded-lg font-bold text-sm ${getGradeColor(suggested)}`}
                                                                    >
                                                                        {formatGradeValue(
                                                                            suggested,
                                                                        )}
                                                                    ...
                                                                    <span
                                                                        className={`inline-flex items-center justify-center px-3 py-1.5 rounded-lg font-bold text-xs ${getBehaviorGradeColor(suggested as string)}`}
                                                                    >
                                                                        {
                                                                            suggested
                                                                        }
                                                        ...
                                                        <td className="px-6 py-6">
                                                            <input
                                                                className={`${gradeMode === "behavior" ? `w-32 text-xs ${getBehaviorGradeColor(proposedGrades[student.id] ?? "")}` : "w-16 bg-surface-container text-on-surface"} border-none focus:ring-2 focus:ring-primary rounded-xl px-3 py-2 text-center font-bold transition-all`}
                                                                type="text"
                                                                value={
                                                                    proposedGrades[
                                                                        student
                                                        ...
                                                        <td className="px-6 py-6">
                                                            <input
                                                                className={`${gradeMode === "behavior" ? `w-32 text-xs ${getBehaviorGradeColor(finalGrades[student.id] ?? (period1 ? period1.wartosc : ""))}` : "w-16 bg-primary-fixed/30 dark:bg-primary/20 text-primary border-2 border-transparent focus:border-primary"} focus:ring-0 rounded-xl px-3 py-2 text-center font-extrabold transition-all`}
                                                                type="text"
                                                                value={
                                                                    finalGrades[
                                                                        student
                                        ...
                                        <div className="mt-4">
                                            ...
                                            <div className="w-full bg-surface-container h-1.5 rounded-full mt-2">
                                                <div
                                                    ...
                                                    style={{
                                                        width: `${filteredStudents.length > 0 ? (Object.keys(finalGrades).length / filteredStudents.length) * 100 : 0}%`,
                                                    }}
                                                />
                                            </div>
                                        </div>
                            ...
                            <div className="mt-4 flex justify-end gap-3">
                                ...
                                <Button
                                    onClick={() => {
                                        ...
                                        createPeriodGradeBatchMutation.mutate(
                                            payload,
                                        );
                                        setFinalGrades({});
                                    }}
                                    ...
                                    disabled={
                                        createPeriodGradeBatchMutation.isPending
                                    }
                                >
                                    {createPeriodGradeBatchMutation.isPending
                                        ? "Zapisywanie..."
                                        : "Zapisz oceny okresowe"}
                                </Button>
                            </div>
                        )}

                        {gradeMode === "behavior" &&
                            Object.keys(pendingBehaviorPoints).length > 0 && (
                    ...
                    <div className="bg-surface-container-lowest rounded-[2rem] shadow-[0_8px_40px_-12px_rgba(0,0,0,0.05)] overflow-hidden">
                        <div className="overflow-x-auto">
                            <table className="w-full text-left border-collapse">
                                <thead>
                                    <tr className="bg-surface-container-low/50">
                                        <th className="px-8 py-5 text-xs font-bold text-on-surface-variant uppercase tracking-wider w-36">
                                            Nr w dziennika
                                        </th>
                                        <th className="px-6 py-5 text-xs font-bold text-on-surface-variant uppercase tracking-wider">
                                            Imię i Nazwisko
                                    ...
                                    {filteredStudents.map(
                                        (student: any, index: number) => (
                                            ...
                                            >
                                                ...
                                                <td className="px-6 py-6 font-headline font-bold text-on-surface">
                                                            ...
                                                            return (
                                                                <>
                                                                    <motion.button
                                                                        ...
                                                                        className={`w-12 h-12 flex items-center justify-center rounded-2xl border-2 transition-all duration-200 shadow-sm bg-background ${isActiveRow ? `border-solid ${getGradeStyles(previewBase, previewMod)}` : `border-dashed ${getGradeStyles(previewBase, previewMod)}`}`}
                                                                    >
                                                                        {!hasPreview ? (
                                                                            <Plus
                                                                                size={
                                                                            ...
                                                                            <AnimatePresence>
                                                                                {activeGradeStudentId ===
                                                                                    ...
                                                                                    <motion.div
                                                                                        ...
                                                                                        style={{
                                                                                            top:
                                                                                                gradeMenuPosition?.top ??
                                                                                                gradeMenuAnchor.top,
                                                                                            left:
                                                                                                gradeMenuPosition?.left ??
                                                                                                gradeMenuAnchor.left,
                                                                                        }}
                                                                                                    ...
                                                                                                    (
                                                                                                        ...
                                                                                                        return (
                                                                                                            <button
                                                                                                                ...
                                                                                                                className={`h-12 rounded-xl text-xl font-bold transition-all border ${isActive ? `${styles} shadow-sm scale-[1.03] z-10 border-2 border-current` : "bg-surface-container-highest text-on-surface-variant font-body hover:bg-surface-container-highest/80 border-border"}`}
                                                                                                            >
                                                                                                                {
                                                                                                                    n
                                                                                                                }
                                                                                                    ...
                                                                                                    (
                                                                                                        ...
                                                                                                        return (
                                                                                                            <button
                                                                                                                ...
                                                                                                                className={`flex-1 rounded-xl text-xl font-bold transition-all disabled:opacity-20 border disabled:cursor-not-allowed ${isActive ? `${styles} shadow-sm scale-[1.03] z-10 border-2 border-current` : "bg-surface-container-highest text-on-surface-variant font-body hover:bg-surface-container-highest/80 border-border"}`}
                                                                                                            >
                                                                                                                {
                                                                                                                    mod
                                                                                                                }
                        ...
                        {Object.keys(pendingGrades).length > 0 && (
                            <div className="p-6 border-t border-outline-variant/20 flex justify-end bg-surface-container-lowest">
                                <Button
                                    onClick={async () => {
                                        const entries =
                                            Object.entries(pendingGrades);
                                        await Promise.all(
            ...
            {canShowStudents && filteredStudents.length === 0 && (
                <EmptyState
                    message={
                        gradeMode === "behavior"
                            ? "Wybierz klasę, aby wystawić punkty zachowania"
                            : "Wybierz przedmiot i klasę, aby wystawić oceny"
                    }
                />
            )}
