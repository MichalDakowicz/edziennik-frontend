  ...
  return (
    <Modal open={open} onClose={onClose} title="Szczegóły oceny">
      <div className="space-y-3 text-sm text-on-surface-variant font-body">
        <div><span className="text-on-surface-variant font-body">Przedmiot:</span> {subjectName}</div>
        <div className="flex items-center gap-2">
          <span className="text-on-surface-variant font-body">Ocena:</span>
        ...
        <div><span className="text-on-surface-variant font-body">Waga:</span> {grade.waga}</div>
        <div><span className="text-on-surface-variant font-body">Kategoria / Opis:</span> {grade.opis ?? "-"}</div>
        <div><span className="text-on-surface-variant font-body">Data wystawienia:</span> {formatDateTime(grade.data_wystawienia)}</div>
